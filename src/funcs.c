...
bool register_fn(VL53L5CX_Configuration* conf, napi_env env, napi_value exports,
                 const char* fn_name, napi_callback cb) {
    napi_value fn;
    napi_status status;
    char error_msg[MAX_LEN_ERROR] = {0};

    status = napi_create_function(env, fn_name, strlen(fn_name), cb, conf, &fn);
    if (status != napi_ok) {
        snprintf(error_msg, MAX_LEN_ERROR - 1, "Could not create JS func %s",
                 fn_name);
        napi_throw_error(env, MODULE_INIT_ERROR, error_msg);
        return false;
    }
...
napi_value cb_vl53l5cx_check_data_ready(napi_env env, napi_callback_info info) {
    size_t argc = MAX_ARGUMENTS;
    napi_value this;
    void* data;
    napi_value ret_val;
    ...
    bool success = parse_args(env, info, &argc, argv, &this, &data, 1, 1);
    if (!success) {
        napi_throw_error(env, ARGUMENT_ERROR, "cb_vl53l5cx_check_data_ready");
        return NULL;
    }
    ...
    uint8_t is_ready = 0;
    uint8_t code = vl53l5cx_check_data_ready(conf, &is_ready);
    if (code) {
        napi_throw_error(env, UNKNOWN_ERROR,
                         "vl53l5cx_check_data_ready failed");
        return NULL;
    }
    is_ready ? napi_get_boolean(env, true, &ret_val)
 ...
 * Get ranging data
 */
napi_value cb_vl53l5cx_get_ranging_data(napi_env env, napi_callback_info info) {
    napi_value this;
    size_t argc = MAX_ARGUMENTS;
    void* data;
    napi_status status;
    ...
    status = napi_create_int32(env, (int32_t)results.silicon_temp_degc,
                               &silicon_temp_c);
    if (status != napi_ok) {
        napi_throw_error(
            env, VALUE_NAPI_ERROR,
    ...
    if (rm_status) {
        napi_throw_error(env, ERROR,
                         "Couldn't get ranging resolution in fn: "
                         "cb_vl53l5cx_get_ranging_data");
        return NULL;
    }
    status = napi_create_array_with_length(env, resolution, /* 4*4 or 8*8 */
                                           &scan_zones);
    if (status != napi_ok) {
        napi_throw_error(
            env, VALUE_NAPI_ERROR,
    ...
    for (uint32_t i = 0; i < resolution; i++) {
        ...
        if (status != napi_ok) {
            napi_throw_error(env, VALUE_NAPI_ERROR,
                             "Couldn't create napi object. fn: "
                             "cb_vl53l5cx_get_ranging_data");
            return NULL;
        }
...
/* Measured distance */
        ...
        status = napi_create_int32(
            env, results.distance_mm[VL53L5CX_NB_TARGET_PER_ZONE * i],
            &distance_mm);
        if (status != napi_ok) {
            napi_throw_error(
                env, VALUE_NAPI_ERROR,
        ...
        status = napi_set_named_property(env, zone, "distanceMillimeters",
                                         distance_mm);
        if (status != napi_ok) {
...
/* Target status */
        ...
        status = napi_create_uint32(
            env, results.target_status[VL53L5CX_NB_TARGET_PER_ZONE * i],
            &target_status);
        if (status != napi_ok) {
            napi_throw_error(env, VALUE_NAPI_ERROR,
                             "Couldn't create napi uint32. fn: "
                             "cb_vl53l5cx_get_ranging_data");
            return NULL;
        }
        ...
        napi_value targets;
        status =
            napi_create_uint32(env, (uint32_t)targets_for_this_zone, &targets);
        if (status != napi_ok) {
            napi_throw_error(env, VALUE_NAPI_ERROR, "fn: get_ranging_data");
            return NULL;
        }
        status =
            napi_set_named_property(env, zone, "nbTargetDetected", targets);
        if (status != napi_ok) {
            napi_throw_error(env, NAMED_PROPERTY_NOT_SET,
                             "fn: get_ranging_data");
            return NULL;
        ...
        status =
            napi_set_named_property(env, zone, "ambientPerSpad", n_ambient);
        if (status != napi_ok) {
            napi_throw_error(env, NAMED_PROPERTY_NOT_SET,
                             "fn: get_ranging_data");
            return NULL;
        ...
        status =
            napi_set_named_property(env, zone, "nbSpadsEnabled", n_num_spads);
        if (status != napi_ok) {
            napi_throw_error(env, NAMED_PROPERTY_NOT_SET,
                             "fn: get_ranging_data");
            return NULL;
        ...
        status = napi_create_array_with_length(env, VL53L5CX_NB_TARGET_PER_ZONE,
                                               &targets_array);
        if (status != napi_ok) {
            napi_throw_error(env, VALUE_NAPI_ERROR, "fn: get_ranging_data");
            return NULL;
        ...
        if (status != napi_ok) {
            napi_throw_error(env, ARRAY_VALUE_NOT_SET,
                             "fn: cb_vl53l5cx_get_ranging_data");
            return NULL;
        }
    }
...
napi_value cb_vl53l5cx_set_resolution(napi_env env, napi_callback_info info) {
    ...
    VL53L5CX_Configuration* conf = ((VL53L5CX_Configuration*)data) + cfg_slot;
 ...
 */
napi_value cb_vl53l5cx_set_ranging_frequency_hz(napi_env env,
...
napi_value cb_vl53l5cx_init(napi_env env, napi_callback_info info) {
    ...
    if (!success) {
        napi_throw_error(env, ARGUMENT_ERROR,
                         "Argument error in cb_vl53l5cx_init");
        return NULL;
    }
...
napi_value cb_vl53l5cx_get_integration_time_ms(napi_env env,
                                               napi_callback_info info) {
    napi_value this;
    size_t argc = MAX_ARGUMENTS;
    void* data;
    ...
    if (res_status) {
        napi_throw_error(env, ERROR_GETTING_VALUE,
...
napi_value cb_vl53l5cx_set_power_mode(napi_env env, napi_callback_info info) {
    ...
    if (status != napi_ok || (power_mode != VL53L5CX_POWER_MODE_SLEEP &&
...
napi_value cb_vl53l5cx_calibrate_xtalk(napi_env env, napi_callback_info info) {
    ...
    bool success = parse_args(env, info, &argc, argv, &this, &data, 4, 4);
    if (!success) { return NULL; }
    ...
    if (status != napi_ok) {
        napi_throw_error(env, ARGUMENT_ERROR,
                         "Invalid argument for calibrate_xtalk(cfg, "
                         "reflectance, n_samples, distance).");
        return NULL;
    }
    ...
    status =
        vl53l5cx_calibrate_xtalk(conf, (uint16_t)reflectance_percent,
                                 (uint8_t)nb_samples, (uint16_t)distance_mm);
    if (status) {
        char err[MAX_LEN_ERROR] = {0};
        snprintf(err, MAX_LEN_ERROR, "Status: 0x%X", status);
        napi_throw_error(env, XTALK_CALIBRATION_FAILED, err);
    }
    return NULL;
}

napi_value cb_vl53l5cx_get_caldata_xtalk(napi_env env,
                                         napi_callback_info info) {
    napi_value this;
    size_t argc = MAX_ARGUMENTS;
    void* data;
    ...
    napi_create_arraybuffer(env, VL53L5CX_XTALK_BUFFER_SIZE, &xtalk_data,
                            &xtalk_data_arraybuf);
    status = vl53l5cx_get_caldata_xtalk(conf, (uint8_t*)xtalk_data);
    if (status) {
        char err[MAX_LEN_ERROR] = {0};
        snprintf(err, MAX_LEN_ERROR, "Status: 0x%X", status);
        napi_throw_error(env, XTALK_CANNOT_FETCH_DATA, err);
        return NULL;
    }
    return xtalk_data_arraybuf;
}

napi_value cb_vl53l5cx_set_caldata_xtalk(napi_env env,
                                         napi_callback_info info) {
    napi_value this;
    size_t argc = MAX_ARGUMENTS;
    void* data;
    ...
    status = napi_get_arraybuffer_info(env, argv[1], (void**)&xtalk_calib,
                                       &xcalib_len);
    if (status) {
        napi_throw_error(env, ARGUMENT_ERROR, "fn: set_caldata_xtalk");
        return NULL;
...
napi_value cb_get_shape_and_kcps_xtalk(napi_env env, napi_callback_info info) {
    ...
    bool success = parse_args(env, info, &argc, argv, &this, &data, 1, 3);
    if (!success) { return NULL; }
